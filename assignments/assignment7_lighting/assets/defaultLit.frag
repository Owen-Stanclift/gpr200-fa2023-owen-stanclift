...
void main(){
	...
	for(i = 0; i < numLights; i++)
	{
		...
		if(_Material.ambientK < 1 && _Material.ambientK > 0)
		{
		vec3 ambient = _Lights[i].color * _Material.ambientK;
		lightColor += ambient;
		}

		if(_Material.diffuseK < 1 && _Material.diffuseK > 0)
		{
		vec3 diffuse = _Lights[i].color * _Material.diffuseK * max(dot(normal, pos),0);
		lightColor += diffuse;
		}

		if(_Material.specular < 1 && _Material.specular > 0 && _Material.shininess > 2 )
		{
